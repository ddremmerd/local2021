...
function getTimeFromBoard() {
    ...
    console.log("get time from")

    const Http = new XMLHttpRequest();
    const url = '/recievedata';
...
function LoadTimeFromBoard() {
    console.log("LOAD TIME FROM BOARD");
}
...
function ConfirmSetTime() {
    ...
    // -----NOTE: ส่ง เวลาและวันไปให้ board
    // console.log("CONFIRM TIME:", datetime_New.hour, datetime_New.minute);
    // console.log("CONFIRM DAY:", datetime_New.wday)

    console.log("POST TO HW:", set_timeHW);
...
function SendNewPass() {
    ...
    if (new_pass.value == '') {
        ...
        if (check_pass == false) {
            ...
            console.log("send new pass to hw--", new_pass.value)
        }
    }
}
...
function DiaplayZone(zoneDec) {
    zoneStatus = DecToBi(zoneDec, maxZone);
...
function GetProgram(clearevt) {
    ...
    if (dataProgram.Data != []) {
        for (let n = 0; n < dataProgram.Data.length; n++) {
            ...
            // return dataProgram_toShowSet
        }
        const sort_prog = dataProgram_toShow_set_2.sort((a, b) => (a.startTime > b.startTime) ? 1 : -1);
        console.log("SORTING PROG:", sort_prog);
...
function disable_displayMode() {
    for (let n = 0; n < dataProgram_toShow_set_2.length; n++) {
        ...
        if (event_con == "showOnly") {
            ...
            // console.log("DISABLE SHOW CLICK")

        } else {
            display_func.removeAttribute("onclick", "ShowDetail(" + n + ")");

            // display_func.setAttribute("onclick", "")
            // console.log("DISABLE SHOW CLICK")
        }
    }
}
...
function HideToggleSwitch() {
    // console.log("COUNT PROGRAM:", countProgram)
...
//------------------ show program after sace program
// function ShowProgram() {

//     console.log("SHOW NEWWWW", dataProgram_toShow_set_2, countProgram)
//     // HideRemoveProgram();
//     // EditButtonForDelete();
//     // addTimer.disabled = false;
//     const sort_prog = dataProgram_toShow_set_2.sort((a, b) => (a.startTime > b.startTime) ? 1 : -1);
//     console.log("SORTING----:", sort_prog)


//     for (let n = 0; n < dataProgram_toShow_set_2.length; n++) {
//         DisplayProgram(n, dataProgram_toShow_set_2[n].hour, dataProgram_toShow_set_2[n].minute, dataProgram_toShow_set_2[n].nZone, dataProgram_toShow_set_2[n].status);
//         let time_array = dataProgram_toShow_set_2[n].startTime;
//         // console.log("SHOW NEWWWW EACH-------", time_array)
//         countProgram++;
//         HideEditButtonForDelete();
        ...
        if (new_start_time == starttime) {
            ...
            for (i = 0; i < dataProgram[n].repeatB.length; i++) {
                ...
                if (dataProgram[n].repeatB[i] == 1) {
                    ...
                    // if (objj.repeatB[i] == 1 && dataProgram[n].repeatB[i] == 1) {
                    //     checkRepeat[i] = 1;
                    //     // let alrtmsg = "เวลาดังกล่าวมีโปรแกรมรดวัน" + dayThaiName[i] + "แล้ว กรุณาเปลี่ยนวันรดซ้ำ"
                    //     // OpenModal(alrtmsg, 'small');
                    //     console.log(n, "เวลาซ้ำ ----- วันซ้ำ", day[i])
                    // }
                    // else if (objj.repeatB[i] == 1 && dataProgram[n].repeatB[i] == 0)
                    // {
                    //     // let alrtmsg = "เวลาดังกล่าวมีโปรแกรมรดวัน" + dayThaiName[i] + "แล้ว กรุณาเปลี่ยนวันรดซ้ำ"
                    //     // OpenModal(alrtmsg, 'small');
                    //     console.log("ไม่ซ้ำ----", day[i])
            ...
            if (new_start_time > starttime && new_start_time < endtime) {
                ...
                // alertMesg = "วัน" + show_day + "มีคิวรดน้ำช่วง  " + H + "." + M + "น. ถึง " + h + "." + m + "น. อยู่แล้ว กรุณาเปลี่ยนเวลาเริ่มรดน้ำ นอกเหนือช่วงเวลาดังกล่าว";
                // OpenModal("เวลาจบทับซ้อนกัน", 'small');
                // console.log(n, "โซนเดียวกัน เวลา      +++จบ+++++     ซ้อนกัน");
            }
...
function AddButton() {
    ...
    if (dataProgram.length == 3) {
        ...
        DisplayTimerSetting('timerProgram', dataProgram_New.hour, dataProgram_New.minute);
        DisplayBacktoMenu();
        timer_part2.style.display = "block";
...
function DeleteTimer(pog_ind) {
    ...
    console.log("DELETE PROGRAM:", pog_ind, "original index: ", dataProgram_toShow_set_2[pog_ind].oriIndex)
    // const update_del = dataProgram_toShow_set_2.splice(pog_ind, 1);
    // console.log("update delete---", update_del, dataProgram_toShow_set_2)
    // UpdateProgram(dataProgram_toShow_set_2);

    // GetProgram('clear');
    // DisplayTimerPart1();
    // ShowProgram(dataProgram_toShow_set_2);

    // console.log("DELETE PROGRAM:", pog_ind)
    if (msg_from_hw == "con_err") {
        ...
        let delete_prog = "{hwId:" + dataProgram_toShow_set_2[pog_ind].oriIndex + "}";
        console.log("delete success:----", delete_prog);
        const update_del = dataProgram_toShow_set_2.splice(pog_ind, 1);
        // console.log("update delete---", update_del, dataProgram_toShow_set_2)
        GetProgram('clear');
        DisplayTimerPart1();
            ...
            if (dataProgram_New.hour >= 0 && dataProgram_New.minute >= 0) {
                ...
                if (dataProgram_New.repeatB != null) {
                    ...
                    if (sum > 0) {
                        ...
                        // console.table(">>>>>", dataProgram_New.repeatB, dataProgram_New.repeatD, dataProgram_New, progIndexNow);

                        //แยกโปรแกรมตามวัน
                        // let dayProgram = ProgramEachDay(event);
                        ...
                        if (ans[0] == true) {
                            ...
                            // dataProgram_toShowSet.push(dataProgram_toShow)
                            // countProgram++;

                            dataProgram_toShow_set_2.push(objjjjjj);
                            // dataProgram_toShowSet.push(objjjjjj);
                            // console.log("บันทึกสำเร็จ------------------", dataProgram);

                            GetProgram('clear');
                            console.log("save data:", dataProgram_toShow_set_2, new_prog)
                            DisplayTimerPart1();
                            // ShowProgram();
                        }
                        else {
...
function PumpControl(pumpIndex) {
    console.log("pump control / send COMMAND", pumpIndex)

    var pump0 = document.getElementById('pump_0');

    //     var clicked = false
    // document.getElementById('pump_0').addEventListener("click", function() {
    //    clicked = true
    // }​);​

    if (pump_man_msg == 'ok') {
        // testPump_status[pIndex] = 1;
        ...
        pump_status = 0;
        OpenModal("CONNECTION LOST", 'big')
    }
    // let id = document.getElementById("pump_" + pumpIndex);
    // let pIndex = parseInt(pumpIndex);
    // let sumP = testPump_status.reduce((a, b) => a + b, 0);
    // let sumV = testValve_status.reduce((a, b) => a + b, 0);

    // if (sumP == 0) {
    //     testPump_status[pIndex] = 1;
    //     id.setAttribute("class", "manual-button manual-on");
    //     PostPumpControl(pIndex, 1);
    // }
    // else if (sumP == 1) {
    //     let found = testPump_status.findIndex(element => element == 1);
    //     if (found != undefined) {
    //         if (found != pIndex) {
    //             OpenModal("ปิดปั๊ม " + (found + 1) + " ก่อน", 'big');
    //         }
    //         else {
    //             testPump_status[pIndex] = 0;
    //             id.setAttribute("class", "manual-button manual-off");
    //             if (sumV == 1) {
    //                 PostValveControl(-1, 0, "bothVP");
    //             }
    //             else {
    //                 PostPumpControl(pIndex, 0);
    //             }
    //         }
...
function ValveControl(val_ind, evt) {
    ...
    if (pump_status == 1) {
        ...
        // for (u = 0; u < valve_status_set.length; u++) {
        //     if (valve_status_set[u] != 1) {
        //         let dis_val = document.getElementById("valve_" + u);
        //         dis_val.setAttribute("class", "manual-button manual-disable");
        //         // dis_val.disabled = true;
        //         // OpenModal("กรุณาปิดวาล์วโซนก่อน", "big");
        //     }
        // }
    }
...
function GetProgramHW() {
    ...
    // dataProgram ชื่อตัวแปรที่ใช้
    if (dataProgram.Data.length != []) {
        if (dataProgram.Data.error == 0) {
...
function GetAllData() {
    ...
    //----- param
    let time_hw;
    let passHW = getSN_Pass();


    console.log("data prgoram:",);
